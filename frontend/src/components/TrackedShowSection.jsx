	...
	return (
		<>
			...
			<div className='tracked-show-header'>
				{trackedShow.showName && <InfoButtonDummy infoButtonDummyClassName={'tracked-show-details-dummy-button'} />}
				<div className='tracked-show-header-show-name'>
					{trackedShow.showName ? <>{trackedShow.showName}</> : <>No show selected</>}
				</div>
			...
			<div
				className='tracker-tracked-show-list-container-wrapper'
				tabIndex='-1'
				onKeyUp={handleKeyUp}
			>
				<div
					className='tracker-tracked-show-list-container'
					tabIndex='-1'
				>
						...
						trackedShow.showStatus === 'In Development' ? (
							<div className='no-show-selected-text-container'>
								...
								{trackedShow.showPremiereDate && (
									<div className='no-show-selected-text'>
										{`It is scheduled to premiere on ${new Date(trackedShow.showPremiereDate).toLocaleDateString()}`}{' '}
										{trackedShow.showAirTime &&
											` at ${new Date(trackedShow.showPremiereDate + 'T' + trackedShow.showAirTime + 'Z').toLocaleTimeString()}`}
										.
									</div>
								)}
								{!trackedShow.showPremiereDate && (
									<>
										<div className='no-show-selected-text'>A premiere date has not yet been released.</div>
										<div className='no-show-selected-text'>Keep checking back for updated information!</div>
									</>
								)}
							</div>
						) : (
							<div className='tracked-show-seasons'>
								{trackedShowSeasons[0] &&
									trackedShowSeasons.map((trackedShowSeason, seasonIndex) => {
										return (
											<Fragment key={`season-row-fragment-${trackedShowSeason.seasonNumber}`}>
												<div
													className='tracked-show-season-row'
													id={`season-row-${trackedShowSeason.seasonNumber}`}
													key={`season-row-${trackedShowSeason.seasonNumber}`}
												>
													<form
														className='tracked-show-season-number-container'
														id={`season-form-${trackedShowSeason.seasonNumber}`}
														key={`season-form-${trackedShowSeason.seasonNumber}`}
														onSubmit={handleToggleSeasonWatched}
													>
														<button
															className='tracked-show-season-number eligible-click-element'
															id={`season-button-${trackedShowSeason.seasonNumber}`}
															key={`season-button-${trackedShowSeason.seasonNumber}`}
															type='submit'
															value={trackedShowSeason._id}
														>
															Season {trackedShowSeason.seasonNumber}
														</button>
													</form>
													<div
														className='tracked-show-season-episodes-container'
														key={`tracked-show-season-episodes-container-${trackedShowSeason.seasonNumber}`}
													>
														{trackedShowSeason.episodeObjectIds.map((episode, episodeIndex) => {
															return (
																<Fragment key={`episode-fragment-${episode._id}`}>
																	<Tippy
																	...
																	>
																		<EpisodeButtonForm
																			...
																			episodeButtonClassName={
																				'tracked-show-season-episode-button eligible-click-element ' +
																				(watchedEpisodes.includes(episode._id) ? 'watched' : 'unwatched')
																			}
																			...
																			episodeButtonInnerDivClassName={
																				'tracked-show-season-episode-button-episode-number'
																			}
																			episodeNumber={episode.episodeNumber}
																			infoButtonClassName={
																				'tracked-show-episode-details-open-button eligible-click-element'
																			}
																			...
																			infoButtonPathClassName={
																				'tracked-show-episode-details-open-button-path ' +
																				(watchedEpisodes.includes(episode._id) ? 'watched' : 'unwatched')
																			}
																			infoButtonPathFillColor={'#A7A7A7'}
																		/>
