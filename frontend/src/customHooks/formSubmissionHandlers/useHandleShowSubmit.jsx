...
const useHandleShowSubmit = async (
  ...
  let suggestionBox = document.querySelector(
    "#tracker-show-add-suggestions-container",
  );
  ...
  try {
    ...
    if (err.response.status && err.response.status === 401) {
      console.log(
        'Caught 401 "unauthorized" error from handleShowSubmit in frontend Tracker.jsx',
        err,
      );
      ...
      toast.error(<Toast icon="⚠️" messageParagraph={message} />, {
        icon: false,
      });
    }
  }
