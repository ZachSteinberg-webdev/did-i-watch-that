...
const useHandleShowSubmit=async(
	e,
	...
	try{
		...
		if(err.response.status && err.response.status===401){
			...
			const message="The external resource that provides TV show data is currently unavailable. Please try again later.";
			toast.error(<Toast icon='⚠️' messageParagraph={message}/>, {icon: false});
		};
	};
	setShowAddShowFormSpinner(false);
	makeAllInputAreasInert(false);
	setTimeout(()=>{
		suggestionBox.childNodes[0]?.scrollIntoView({behavior: 'smooth', block: "end", inline: "nearest"});
	},200);
	setTimeout(()=>{
