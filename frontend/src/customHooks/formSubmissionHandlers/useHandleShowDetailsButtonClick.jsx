...
const useHandleShowDetailsButtonClick = async (
  ...
  if (e.target.classList.contains("add-show-result-details-open-button")) {
    ...
    try {
      ...
      showData.showImageLink =
        showDataObject.image && showDataObject.image.original;
      showData.showLanguage = showDataObject.language;
      showData.showNetwork =
        showDataObject.network && showDataObject.network.name;
      showData.showNetworkWebsiteLink =
        showDataObject.network && showDataObject.network.officialSite;
      showData.showCountry =
        showDataObject.network && showDataObject.network.country.name;
      ...
      showData.showSummaryHtml = sanitizeHtml(showDataObject.summary, {
        allowedTags: ["p"],
        allowedAttributes: {},
      });
      showData.showType = showDataObject.type;
      showData.showUpdatedEpoch = showDataObject.updated;
      showData.showWebChannelName =
        showDataObject.webChannel && showDataObject.webChannel.name;
      showData.showWebChannelWebsiteLink =
        showDataObject.webChannel && showDataObject.webChannel.officialSite;
      setTrackedShowDetails(showData);
      handleTrackedShowDetailsModalOpen();
    } catch (err) {
      if (err.response && err.response.status === 401) {
        console.log(
          'Caught 401 "unauthorized" error from handleShowDetailsButtonClick in frontend Tracker.jsx',
          err,
        );
        unauthorized401Response(navigate);
      } else {
        console.log(
          "Caught error from handleShowDetailsButtonClick in frontend Tracker.jsx",
          err,
        );
