		...
		if (showObjectId === currentlyTrackedShowObjectId) {
			...
			toast.success(
				<Toast
					icon='🤦‍♂️'
					icon2='🤦‍♀️'
					messageParagraphTwoIcons={message}
				/>,
				{ icon: false }
			);
		} else {
			...
			if (guestMode) {
				...
				const seasonsRes = await axios.get(`https://api.tvmaze.com/shows/${show.showId}/seasons`);
				const seasonsData = seasonsRes.data;
				const seasons = [];
				for (const season of seasonsData) {
					const epRes = await axios.get(`https://api.tvmaze.com/seasons/${season.id}/episodes`);
