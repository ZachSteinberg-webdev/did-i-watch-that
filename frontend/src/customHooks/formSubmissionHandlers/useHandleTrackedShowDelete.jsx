  ...
  try {
    if (guestMode) {
      ...
      const next = existing.filter(
        (s) => String(s._id) !== String(showObjectId),
      );
      ...
      toast.success(
        <Toast icon="👍" messageParagraph={"Show removed from your list."} />,
        { icon: false },
      );
      useRemoveTrackedShowFromState(
        setTrackedShows,
        trackedShows,
        showObjectId,
      );
    ...
    if (err.response.status && err.response.status === 401) {
      console.log(
        'Caught 401 "unauthorized" error from handleTrackedShowDelete in frontend Tracker.jsx',
        err,
      );
      ...
      console.log(
        "Caught error from handleTrackedShowDelete in frontend Tracker.jsx",
        err,
      );
