...
const useHandleToggleEpisodeWatched = async (
  ...
  const episodeAirstamp = new Date(
    episodeJsonObject.episodeAirstamp || episodeJsonObject.airstamp,
  );
  const currentDatestamp = new Date();
  if (episodeAirstamp >= currentDatestamp) {
    const seasonNumber =
      episodeJsonObject.seasonNumber || episodeJsonObject.season;
    const episodeNumber =
      episodeJsonObject.episodeNumber || episodeJsonObject.number;
    ...
    const showEpisodeDetailsButtonSvgPath =
      e.target[2].firstChild.firstChild.firstChild.firstChild.firstChild
        .firstChild.firstChild.firstChild.firstChild.firstChild;
    toggleEpisodeWatchedClassList([
      episodeButton,
      showEpisodeDetailsButtonSvgPath,
    ]);
    try {
      if (guestMode) {
        ...
        useUpdateWatchedEpisodesState(setWatchedEpisodes, watchedEpisodes, [
          episodeObjectId,
        ]);
        const seasonNumber =
          episodeJsonObject.seasonNumber || episodeJsonObject.season;
        const episodeNumber =
          episodeJsonObject.episodeNumber || episodeJsonObject.number;
        const episodeTitle =
          episodeJsonObject.episodeTitle || episodeJsonObject.name;
        const watchedStatus = arr.includes(episodeObjectId)
          ? "watched"
          : "unwatched";
      ...
      if (err.response && err.response.status === 401) {
        ...
        console.log(
          "Caught error from handleToggleEpisodeWatched in frontend Tracker.jsx",
          err,
        );
