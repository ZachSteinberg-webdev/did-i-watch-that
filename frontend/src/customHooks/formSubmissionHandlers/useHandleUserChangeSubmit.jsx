...
const useHandleUserChangeSubmit=async(
	e,
	name,
	email,
	oldPassword,
	...
	setUser
		...
		try{
			...
			if(userDetailData.data.success === true){
				...
				toast.success(<Toast icon='👍' messageParagraph={userDetailsUpdateMessage}/>, {icon: false});
			};
		}catch(err){
			console.log('err from Dashboard.jsx frontend', err);
			if(err.response.data.error){
				let messageArray = err.response.data.error;
				toast.error(<Toast icon='👎' messageHeader='Update failed:' messageArray={messageArray}/>, {icon: false});
			}else{
				toast.error(<Toast icon='👎' messageParagraph='Update failed. Please try again later.'/>, {icon: false});
			};
		};
		try{
			...
			if(userPasswordData.data.success===true){
				...
				toast.success(<Toast icon='👍' messageParagraph={userPasswordUpdateMessage}/>, {icon: false});
			};
		}catch(err){
			console.log('err from Dashboard.jsx frontend', err);
			if(err.response.data.error){
