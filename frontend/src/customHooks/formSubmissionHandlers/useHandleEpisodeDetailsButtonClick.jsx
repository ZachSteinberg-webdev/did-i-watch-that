...
const useHandleEpisodeDetailsButtonClick = async (
  ...
  try {
    if (guestMode) {
      ...
      try {
        ...
        const btn = document.getElementById(
          `tracked-show-season-episode-button-${episodeValue}`,
        );
        if (btn) {
      ...
      if (episodeObj) {
        ...
        const currentShow = shows.find(
          (s) => s.showName === episodeObj.showName,
        );
        ...
        await setHoveredEpisodeDetails({
          ...
          episodeRating:
            episodeObj.episodeRating ||
            (episodeObj.rating && episodeObj.rating.average),
          episodeSummaryHtml:
            episodeObj.episodeSummaryHtml || episodeObj.summary,
          episodeImageLink:
            episodeObj.episodeImageLink ||
            (episodeObj.image && episodeObj.image.original),
        });
        setTimeout(() => {
          handleEpisodeDetailsModalOpen();
        }, 300);
    ...
    if (err.response && err.response.status === 401) {
      ...
      console.log(
        "Caught error from handleToggleEpisodeWatched episodeDetailsButton in frontend Tracker.jsx",
        err,
      );
