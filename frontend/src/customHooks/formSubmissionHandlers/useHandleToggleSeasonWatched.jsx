...
const useHandleToggleSeasonWatched = async (e, user, trackedShow, setWatchedEpisodes, watchedEpisodes, navigate) => {
	e.preventDefault();
	...
	try {
		if (guestMode) {
			...
			const episodeForms = seasonRow?.querySelectorAll('form.tracked-show-season-episode-container');
			...
			const alreadyWatchedAll = allEpisodeIds.every((id) => storedEpisodeIds.includes(id));

			if (alreadyWatchedAll) {
				storedEpisodeIds = storedEpisodeIds.filter((id) => !allEpisodeIds.includes(id));
				watchedMap[showKey] = storedEpisodeIds;
				localStorage.setItem('guestWatchedEpisodes', JSON.stringify(watchedMap));
				useUpdateWatchedEpisodesState(setWatchedEpisodes, watchedEpisodes, allEpisodeIds, 'unwatched');
				...
				const merged = Array.from(new Set([...storedEpisodeIds, ...airedEpisodeIds]));
				watchedMap[showKey] = merged;
				localStorage.setItem('guestWatchedEpisodes', JSON.stringify(watchedMap));
				useUpdateWatchedEpisodesState(setWatchedEpisodes, watchedEpisodes, airedEpisodeIds, 'watched');
				const suffix = airedEpisodeIds.length < allEpisodeIds.length ? ', except those which have not aired yet' : '';
