...
const syncShowContent = async (show) => {
	...
	for (const season of seasonsData) {
		...
		const seasonEpisodesData = await axios.get(`https://api.tvmaze.com/seasons/${season.id}/episodes`);
		...
		for (const episode of episodesData) {
			...
			if (!seasonDoc.episodeObjectIds.some((id) => idsMatch(id, episodeDoc._id))) {
...
const addShowForUser = async (userId, postedShowData) => {
	...
	if (existingShowObjectId && listOfShowsTrackedByUser.some((showId) => idsMatch(showId, existingShowObjectId))) {
		return {
			success: false,
	...
	if (existingShowObjectId) {
		...
		if (showLastUpdatedByApiEpoch > showLastUpdatedInDatabaseEpoch) {
			...
			existingShow.showCountry =
				postedShowData.network && postedShowData.network.country && postedShowData.network.country.name;
			existingShow.showWebsiteLink = postedShowData.officialSite;
			existingShow.showAverageRating = postedShowData.rating && postedShowData.rating.average;
			existingShow.showAirDays = postedShowData.schedule && postedShowData.schedule.days;
			existingShow.showAirTime = postedShowData.schedule && postedShowData.schedule.time;
		...
		if (!listOfShowsTrackedByUser.some((showId) => idsMatch(showId, existingShowObjectId))) {
		...
		return {
			success: true,
			message: [`"${existingShow.showName || postedShowData.name}" has been added to your list of tracked shows!`],
	...
	const newShow = {
		...
		showNetworkWebsiteLink: postedShowData.network && postedShowData.network.officialSite,
		showCountry: postedShowData.network && postedShowData.network.country && postedShowData.network.country.name,
		...
		showWebChannelName: postedShowData.webChannel && postedShowData.webChannel.name,
		showWebChannelWebsiteLink: postedShowData.webChannel && postedShowData.webChannel.officialSite,
		seasonObjectIds: []
	};
	...
	return {
		success: true,
		message: [`"${createdShow.showName || postedShowData.name}" has been added to your list of tracked shows!`],
		returnedListOfTrackedShows,
		show: createdShow
	};
};
