...
exports.updatePassword = async (req, res, next) => {
	...
	if (!oldPassword && !newPassword) {
		...
		return next(new ErrorResponse(['Unable to update password. Please try again later.'], 500));
	}
};
...
exports.toggleDarkMode = async (req, res, next) => {
	...
	try {
		...
		return next(new ErrorResponse(["We couldn't update your display preference. Please try again."], 500));
	}
};
...
exports.migrateGuestData = async (req, res, next) => {
	try {
		...
		if (!userId) {
			return next(new ErrorResponse(['Please log in to migrate guest data.'], 401));
		}
		...
		for (const showIdString of Array.from(unifiedShowIds)) {
			...
			if (Number.isNaN(tvmazeId)) {
				return next(new ErrorResponse([`Guest migration failed because show id "${showIdString}" is invalid.`], 400));
			}
			const tvmazeResponse = await axios.get(`https://api.tvmaze.com/shows/${tvmazeId}`);
			...
			const watchedEpisodeIdsRaw =
				watchedEpisodesMap[showIdString] || watchedEpisodesMap[tvmazeId] || watchedEpisodesMap[String(tvmazeId)] || [];
			if (Array.isArray(watchedEpisodeIdsRaw) && watchedEpisodeIdsRaw.length > 0) {
				const watchedEpisodeIds = watchedEpisodeIdsRaw.map((id) => Number(id)).filter((id) => !Number.isNaN(id));
				if (watchedEpisodeIds.length > 0) {
					const episodes = await Episode.find({
						episodeId: { $in: watchedEpisodeIds }
					});
					for (const episode of episodes) {
						if (!episode.usersWhoHaveWatched.some((userObjectId) => String(userObjectId) === String(userId))) {
		...
		res.status(200).json({ success: true, showsList: showsList || { shows: [] } });
	} catch (err) {
		return next(err);
	}
};
